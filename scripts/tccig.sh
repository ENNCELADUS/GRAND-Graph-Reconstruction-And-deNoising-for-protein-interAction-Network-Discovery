...
if [ ! -e "$TARGET_PATH" ]; then
  echo "Missing TCCIG config target: ${TARGET_PATH}" >&2
  echo "Pass a config file/directory, or set GRAND_TCCIG_CONFIG." >&2
  exit 1
fi
...
if [ "$NGPUS" -lt 1 ]; then
  echo "No GPUs detected in the SLURM allocation." >&2
  exit 1
fi
