...
validation/test truth 不进入 backward。loss 使用已有可微 surrogate，并且 backward
只启用 graph similarity 和 relative density；`gamma` / `delta` 即使配置了也不会进入
训练侧可微 loss：
...
monitor，不作为 test-time threshold search。后续如需更强正则，可以再评估 soft
degree/clustering terms 或 soft distillation：
...
pairwise candidate pairs Ω_pairwise = human_test_ppi.txt
topology candidate pairs Ω_topology = all_test_ppi.txt
```

pairwise test 流程：

```text
1. s_ij_pairwise = Cφ_all(x_i, x_j), for all (i,j) in human_test_ppi.txt
...
5. report full-model binary metrics at the fixed refined output threshold
```
