...
The baseline algorithm below describes the intended training design. The current loss behavior and the recommended fixes for the active weighted-sum plateau are tracked separately in [issue.md](/Users/richardwang/Documents/grand/docs/topology_finetune/issue.md).

Current short version:

- the model is stuck in an over-dense regime
- `RD loss` dominates the topology term numerically
- recommended fixes are: effective warmup, stable RD, topology-loss normalization, then optional grouped GradNorm
